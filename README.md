# FlowCore

> 계획 기반 AI 엔지니어링 워크플로우 오케스트레이션 시스템

FlowCore는 AI를 활용한 소프트웨어 개발을 위한 workflow architecture 입니다.

핵심 목표는:
- workflow orchestration
- agent 역할 분리
- review governance
- sequential PR integrity
- runtime-risk-oriented validation
- prompt compilation

입니다.

FlowCore의 목표는 AI를 “더 똑똑하게” 만드는 것이 아닙니다.

대신 엔지니어링 workflow를:
- 재현 가능하게
- 리뷰 가능하게
- 안정적으로
- low-risk 하게
- 운영 가능하게

만드는 것입니다.

---

# Philosophy

FlowCore는 하나의 핵심 원칙 위에서 설계되었습니다.

> 좋은 AI 엔지니어링은  
> 좋은 프롬프트가 아니라  
> 좋은 workflow architecture 에서 나온다.

FlowCore는 다음을 중요하게 생각합니다.

- 구현보다 계획 우선
- 역할 분리
- strict scope discipline
- runtime safety
- review quality
- orchestration consistency

FlowCore는 프롬프트를 단순 채팅 입력이 아니라:

```text
execution artifact
```

로 다룹니다.

---

# Core Concepts

## 1. Plan-Driven Development

모든 구현은 계획(plan)에서 시작됩니다.

```text
context normalize
→ discover
→ spec
→ plan
→ implement
→ review
→ advisory review
→ reconciliation
→ runtime validation
→ commit
→ PR
```

구현 agent는:
- 계획 범위를 벗어나지 않고
- scope expansion 을 피하며
- architecture drift 를 방지하고
- speculative refactoring 을 하지 않아야 합니다.

---

## 2. Agent Responsibility Separation

각 agent는 서로 다른 역할을 가집니다.

| 역할 | 책임 |
|---|---|
| Architect | 계획 및 workflow 설계 |
| Implementer | 구현 |
| Reviewer | correctness/runtime validation |
| Advisory Reviewer | edge-case/runtime inconsistency 탐지 |
| Reconciliation Authority | 최종 판단 |

예시 orchestration:

```text
Codex
→ architecture planning

Claude
→ implementation

Codex
→ review validation

Gemini
→ advisory runtime review

Codex
→ final reconciliation
```

---

## 3. Workflow State as Files

FlowCore는 파일 자체를 workflow state 로 사용합니다.

```text
ai/
  context.md
  discovery.md
  spec.md
  plan.md
  review.md
  gemini-review.md
  runtime-findings.md
  review-summary.md
  pr-review-plan.md
```

workflow는 chat history 가 아니라:
- 계획 문서
- 리뷰 결과
- review queue
- execution state

를 기반으로 동작합니다.

---

## 4. Prompt Compilation

사람이 긴 프롬프트를 반복 작성하지 않도록 설계합니다.

FlowCore는:
- workflow state
- current PR
- current task
- review queue
- execution context

를 기반으로 프롬프트를 자동 생성하는 방향을 목표로 합니다.

예시 command:

```bash
fc plan
fc implement
fc review
fc gemini
fc reconcile
fc summary
fc commit
```

workflow compiler 는:
- template 선택
- context 주입
- execution rule 유지
- workflow boundary enforcement

를 담당합니다.

---

# Workflow Visualization

FlowCore의 기본 실행 흐름은 계획, 구현, 리뷰, 검증, 배포 준비를 분리합니다.

```mermaid
flowchart TD
    A[Request / Issue / Slack / Jira / Screenshot] --> B[Context Normalize]
    B --> C[Discover]
    C --> D[Spec Draft]
    D --> E[Plan Draft]
    E --> F{Human approves plan?}
    F -- No --> E
    F -- Yes --> G[Implementation]
    G --> H[Primary Review]
    H --> I[Advisory Review]
    I --> J[Reconciliation]
    J --> K{Blocking issue?}
    K -- Yes --> L[Fix Implementation]
    L --> H
    K -- No --> M[Runtime Validation]
    M --> N{Runtime OK?}
    N -- No --> O[Runtime Findings]
    O --> P{Root cause unclear?}
    P -- Yes --> C
    P -- No --> L
    N -- Yes --> Q[Commit]
    Q --> R[PR]
    R --> S[CI / ECC Gate]
    S --> T{Human merge approval?}
    T -- No --> J
    T -- Yes --> U[Merge]
```

## Context Normalize

`Context Normalize`는 Slack, Jira, screenshot, voice-like text 같은 입력을 구조화된 engineering request 로 바꾸는 단계입니다.

이 단계의 출력은 `ai/context.md`입니다.

목표:
- 원문 요청과 출처를 보존
- confirmed fact, inferred assumption, open question 을 분리
- product/runtime expectation 을 명확히 정리
- Discover 가 바로 탐색할 수 있는 scope hint 와 evidence link 를 제공

이 단계는 설계나 구현 결정을 내리지 않습니다.

---

## Review And Runtime Loops

Review loop 와 runtime loop 는 서로 다른 피드백 루프입니다.

Review loop:

```text
Primary Review
→ Advisory Review
→ Reconciliation
→ Blocking?
→ Fix Implementation
→ Primary Review
```

Runtime loop:

```text
Runtime Validation
→ Runtime Findings
→ Root cause unclear?
→ Discover 또는 Fix Implementation
→ Primary Review
```

원칙:
- reconciliation 은 판단 단계입니다.
- blocking issue 수정은 `Fix Implementation`에서 수행합니다.
- runtime failure 는 먼저 `ai/runtime-findings.md`로 정리합니다.
- runtime failure 의 원인이 기존 가정 오류라면 Discover 로 재진입합니다.
- 원인이 명확한 구현 결함이면 Fix Implementation 으로 돌아갑니다.

---

# Workflow State Machine

FlowCore는 나중에 `fc next`가 판단할 수 있도록 workflow state machine 으로 확장될 수 있습니다.

예시 state:

```yaml
state:
  current: review
  previous: implementation
  blocked: false
  plan_approved: true
  runtime_verified: false
  merge_approved: false
artifacts:
  context: ai/context.md
  discovery: ai/discovery.md
  spec: ai/spec.md
  plan: ai/plan.md
  primary_review: ai/review.md
  advisory_review: ai/gemini-review.md
  runtime_findings: ai/runtime-findings.md
next:
  command: fc reconcile
  reason: primary and advisory reviews are available
```

핵심은 chat history 가 아니라 artifact 와 state transition 을 기준으로 다음 행동을 결정하는 것입니다.

## User Scenario

사용자는 workflow gate를 열고 닫는 역할을 맡고, AI는 승인된 구간에서 문서를 읽고 결과 파일을 생성합니다.

```mermaid
sequenceDiagram
    actor Human
    participant FlowCore
    participant AI as AI Agent
    participant State as ai/*.md
    participant Git

    Human->>FlowCore: 작업 요청 입력
    FlowCore->>AI: context normalize / discover / spec draft / plan draft 실행
    AI->>State: context.md, discovery.md, spec.md, plan.md 작성
    FlowCore-->>Human: 계획 검토 요청
    Human->>FlowCore: 구현 승인 또는 수정 지시

    alt 구현 승인
        FlowCore->>AI: 승인된 plan 기준 구현 지시
        AI->>Git: 코드 변경 생성
        FlowCore->>AI: /review current 실행
        AI->>State: review.md 작성
        FlowCore-->>Human: 리뷰 결과와 runtime 확인 요청
        Human->>FlowCore: 수정 요청, runtime finding, 또는 runtime 승인
    else 구현 미승인
        FlowCore->>AI: plan/spec 재작성
        AI->>State: plan.md 또는 spec.md 갱신
    end

    opt commit / PR / merge 단계
        Human->>FlowCore: commit 승인
        FlowCore->>Git: commit 생성
        Human->>FlowCore: PR 생성 승인
        FlowCore->>Git: PR 생성
        Human->>FlowCore: merge 승인
        FlowCore->>Git: merge
    end
```

---

# Review Architecture

FlowCore는 리뷰 역할을 강하게 분리합니다.

## Primary Review

중점:
- correctness
- architecture consistency
- runtime safety
- regression prevention

---

## Advisory Review

중점:
- edge cases
- runtime inconsistencies
- lifecycle issues
- sequential PR risks
- hidden coupling

---

## Final Reconciliation

중점:
- actionable decisions
- severity validation
- merge readiness
- review governance

Final Reconciliation 은 수정 자체를 수행하지 않습니다.

결과는 다음 중 하나로 이어져야 합니다.

- blocking issue 없음 → runtime validation
- blocking issue 있음 → fix implementation
- review 근거 부족 → 필요한 artifact 보강

---

# Runtime Findings

Runtime validation 실패는 review finding 과 분리합니다.

`ai/runtime-findings.md`에는 다음을 기록합니다.

- 재현 경로
- 기대 동작
- 실제 동작
- runtime evidence
- root cause clarity: `clear | unclear`
- 기존 discovery/spec/plan 가정이 틀렸는지 여부
- 다음 전이: `discover | fix-implementation | runtime-retry`

runtime failure 가 다음에 해당하면 Discover 로 재진입합니다.

- API contract assumption 이 틀림
- lifecycle assumption 이 틀림
- cache invalidation assumption 이 틀림
- product/runtime expectation 자체가 불명확함

---

# Sequential PR Support

FlowCore는:
- stacked PR
- sequential feature delivery
- migration workflow
- dependency-aware review execution

을 고려하여 설계되었습니다.

시스템은:
- PR isolation
- runtime consistency
- review ordering
- cross-PR validation

을 유지하려고 합니다.

---

# Design Principles

## Scope Discipline

다음을 방지하는 것을 중요하게 생각합니다.

- mixed-scope commit
- speculative cleanup
- architecture drift
- unnecessary abstraction
- hidden coupling

---

## Runtime Safety First

리뷰는 다음을 우선 검증합니다.

- lifecycle correctness
- async timing stability
- rendering consistency
- state synchronization
- performance-sensitive flow
- regression prevention

---

## Human-in-the-Loop

사람은 여전히 다음 책임을 가집니다.

- runtime validation
- product decision
- merge approval
- workflow supervision

FlowCore는 엔지니어링 workflow를 보조합니다.

사람의 엔지니어링 판단을 대체하지 않습니다.

---

# Current Focus Areas

현재 주요 관심 영역:

- iOS UIKit workflow
- AI-assisted review system
- sequential PR orchestration
- workflow-driven development
- prompt compilation
- runtime-risk-oriented review pipeline

---

# Future Directions

향후 방향성:

- workflow DSL
- command runtime
- prompt compiler
- PR dependency graph
- automated review queue orchestration
- runtime validation checklist
- workflow state engine
- multi-agent execution runtime

---

# Example Workflow

## Bug Fix Workflow

```text
Slack request
→ context normalize
→ discover
→ spec
→ plan
→ implementation
→ review
→ advisory review
→ reconciliation
→ fix implementation if blocking
→ runtime validation
→ runtime findings if failed
→ commit
→ PR
```

---

## Sequential UI Workflow

```text
UI item 1
→ plan
→ implement
→ review
→ commit

UI item 2
→ repeat
```

원칙:
- one item per commit
- one review lifecycle per PR

---

# Non-Goals

FlowCore는 다음을 목표로 하지 않습니다.

- autonomous coding agent
- no-human coding system
- prompt collection repository
- architecture replacement tool

FlowCore는:

```text
engineering workflow system
```

입니다.

---

# License

MIT
